...
def parse_arguments():
    ...
    parser.add_argument('--pin-skyloc', action='store_true', help='Pins RA, DEC, tref to their true values for ILE calculation')
    ...
    parser.add_argument("--test", action='store_true', help='Uses test versions of RIFT code pieces')
    parser.add_argument('--taper-domain', action='store_true', help='Changes time series plot range to show tapering')
    parser.add_argument('--check-frame-duplicate', action='store_true', help='gens waves with hoft like LALWriteFrame')
...
for ifo in ifos:
    ...
    PSD_paths[ifo] = f'/home/chad.henshaw/Injections/PSDs/O4/{ifo}-psd.xml.gz'
    channels[ifo] = f'{ifo}:FAKE-STRAIN'
    
# PSDs for Plotting
LIGO_PSD = lalsimutils.get_psd_series_from_xmldoc(PSD_paths['H1'], 'H1')
...
def gen_wav_old(P_dict, name):
    ...
    for ifo in P_dict.keys():
        ...
        wav_data[ifo]['hoff'] = np.abs(hoff.data.data)
        wav_data[ifo]['freqs'] = np.array([hoff.f0 + i * hoff.deltaF for i in range(hoff.data.length)])
...
def gen_wav(P_dict, name):
    ...
    for ifo in P_dict.keys():
        ...
        if args.check_frame_duplicate:
            ...
            hlmF, _= factored_likelihood.internal_hlm_generator(P_copy, lmax, extra_waveform_kwargs=extra_waveform_args)
            hlmT = {}
            for mode in hlmF:
                hlmT[mode] = lalsimutils.DataInverseFourier(hlmF[mode])

            P_copy = P_dict[ifo].manual_copy()
            hoft = lalsimutils.hoft_from_hlm(hlmT, P_copy,return_complex=False)
...
def run_ILE(xml, output_name):
    ...
    #if args.test:
    #    options.append('--data-integration-window-half 0.02')
    ...
    # ILE exe path:
    if args.test:
        ILE_path = os.path.join(os.path.dirname(sys.executable), 'integrate_likelihood_extrinsic_batchmode_test')
    else:
...
P_truth_chip = P_truth['H1'].extract_param('chi_p')
P_truth_chiprms = P_truth['H1'].extract_param('chi_prms')
...
if args.user_test:
    ...
    P_test_chip = P_test['H1'].extract_param('chi_p')
    P_test_chiprms = P_test['H1'].extract_param('chi_prms')
...
if args.maxL:
    ...
    P_maxL_chip = P_maxL['H1'].extract_param('chi_p')
    P_maxL_chiprms = P_maxL['H1'].extract_param('chi_prms')
...
if args.calc_ILE:
    ...
    if args.maxL and args.user_test:        
        ...
        result_file = os.path.join(diag_path, f'Injection+maxL+test_mtot-{(P_test["H1"].m1+P_test["H1"].m2)/SM}_q-{P_test["H1"].m1/P_test["H1"].m2}_chi1-{P_test["H1"].s1z}_chi2-{P_test["H1"].s2z}_lmax-{lmax}_ILEresults.txt')
        ...
        result_file = os.path.join(diag_path, f'Injection+test_mtot-{(P_test["H1"].m1+P_test["H1"].m2)/SM}_q-{P_test["H1"].m1/P_test["H1"].m2}_chi1-{P_test["H1"].s1z}_chi2-{P_test["H1"].s2z}_lmax-{lmax}_ILEresults.txt')
    else:
    ...
    if args.maxL:
        # calculate lnL gap
        lnL_gap = truth_lnL - maxL_lnL
        result_list.append(lnL_gap)
        
    if args.user_test and not(args.maxL):
        # calculate lnL gap
        lnL_gap = truth_lnL - test_lnL
        result_list.append(lnL_gap)
        
    if args.user_test and args.maxL:
        lnL_gap_maxL = truth_lnL - maxL_lnL
        result_list.append(lnL_gap_maxL)
        lnL_gap_test = truth_lnL - test_lnL
        result_list.append(lnL_gap_test)
...
if not(args.no_plots):
    ...
    for ifo in ifos:
        ...
        ax[1].loglog(wav_data_truth[ifo]['freqs'], wav_data_truth[ifo]['hoff'], color='blue', linestyle='dashed', label=f'True params')
        ...
        if args.user_test:
            ...
            ax[1].loglog(wav_data_test[ifo]['freqs'], wav_data_test[ifo]['hoff'], color='green', linestyle='dotted', alpha=0.5, label=f'Test params')
        ...
        if args.maxL:
            ...
            ax[1].loglog(wav_data_maxL[ifo]['freqs'], wav_data_maxL[ifo]['hoff'], color='red', linestyle='dashdot', alpha=0.5, label=maxL_label)
        ...
        ax[0].set_xlim(-13.0 + 1e9, 0.1 + 1e9)
        if args.taper_domain:
            ax[0].set_xlim(-15.0 + 1e9, 15.0 + 1e9)
        ...
        ax[1].set_xlim(5.0, 4000.0) # for now
        ...
        textstr_inj = '\n'.join((
            ...
            '$\chi_p=%.2f$' % (P_truth_chip),
            r'$\sqrt{\langle \chi_p^2 \rangle}=%.2f$' % (P_truth_chiprms)))
        ...
        if args.user_test:
            textstr_test = '\n'.join((
                ...
                '$\chi_p=%.2f$' % (P_test_chip),
                r'$\sqrt{\langle \chi_p^2 \rangle}=%.2f$' % (P_test_chiprms)))
        ...
        if args.maxL:
            textstr_maxL = '\n'.join((
                ...
                '$\chi_p=%.2f$' % (P_maxL_chip),
                r'$\sqrt{\langle \chi_p^2 \rangle}=%.2f$' % (P_maxL_chiprms)))
        ...
        if args.calc_ILE:
            if args.maxL and args.user_test:
                savepath = os.path.join(
                    diag_path,
                    f"Injection+maxL+test_mtot-{(P_test[ifo].m1 + P_test[ifo].m2) / SM}_q-{P_test[ifo].m1 / P_test[ifo].m2}_chip-{P_test_chip}_chiprms-{P_test_chiprms}_lmax-{lmax}_ILEresults_{ifo}-frame.png"
                )
            elif args.maxL and not args.user_test:
                savepath = os.path.join(
                    diag_path,
                ...
                savepath = os.path.join(
                    diag_path,
                    f"Injection+test_mtot-{(P_test[ifo].m1 + P_test[ifo].m2) / SM}_q-{P_test[ifo].m1 / P_test[ifo].m2}_chip-{P_test_chip}_chiprms-{P_test_chiprms}_lmax-{lmax}_ILEresults_{ifo}-frame.png"
                )
            else:
                savepath = os.path.join(
            ...
            if args.maxL and args.user_test:
                savepath = os.path.join(
                    diag_path,
                    f"Injection+maxL+test_mtot-{(P_test[ifo].m1 + P_test[ifo].m2) / SM}_q-{P_test[ifo].m1 / P_test[ifo].m2}_chip-{P_test_chip}_chiprms-{P_test_chiprms}_lmax-{lmax}_NoILE_{ifo}-frame.png"
                )
            elif args.maxL and not args.user_test:
                savepath = os.path.join(
                    diag_path,
                ...
                savepath = os.path.join(
                    diag_path,
                    f"Injection+test_mtot-{(P_test[ifo].m1 + P_test[ifo].m2) / SM}_q-{P_test[ifo].m1 / P_test[ifo].m2}_chip-{P_test_chip}_chiprms-{P_test_chiprms}_lmax-{lmax}_NoILE_{ifo}-frame.png"
                )
            else:
                savepath = os.path.join(
